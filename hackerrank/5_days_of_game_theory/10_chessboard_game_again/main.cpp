...
ll dfs(int x, int y) {
  ...
  set<ll> s;
  for (auto diff : moves) {
    int nx = x + diff.first, ny = y + diff.second;
...
int main(int argc, char* argv[]) {
  ...
  rep(i, 0, t) {
    ...
    vector<ll> gs(n);
    rep(i, 0, n) gs[i] = dfs(coins[i].first, coins[i].second);
    put(Nimk(1, gs) ? "First" : "Second");
  }
