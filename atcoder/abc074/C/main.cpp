...
int main(int argc, char* argv[]) {
  ...
  for (ll water1 = 0; water1 <= f; water1 += a * 100) {
    for (ll water2 = 0; water1 + water2 <= f; water2 += b * 100) {
      ll total_sugar = min(f - (water1 + water2), ((water1 + water2) / 100) * e);

      ll max_sugar = 0;
      for (ll sugar1 = 0; sugar1 <= total_sugar; sugar1 += c) {
        ll rest_sugar = total_sugar - sugar1;
        ll sugar = total_sugar - (rest_sugar % d);
        chmax(max_sugar, sugar);
      }

      if (water1 + water2 + max_sugar == 0) continue;
      double cur_density = double(max_sugar) / (water1 + water2 + max_sugar);
      if (density >= cur_density) continue;
      ans.first = water1 + water2 + max_sugar;
      ans.second = max_sugar;
      density = cur_density;
    }
  }
