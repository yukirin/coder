...
int main(int argc, char* argv[]) {
  ...
  vector<ll> bits(64);
  rrep(i, 63, 0) rep(j, 0, n) {
    if (a[j] & (1LL << i)) bits[i]++;
  }

  Fill(dp, ll(-1));
  dp[0][0] = 0;
  rep(i, 0, 61) rep(smaller, 0, 2) {
    ll base = (1LL << (60 - i));
    ll flag = bool(k & base);

    if (!smaller) {
      ll count = flag ? n - bits[60 - i] : bits[60 - i];
      dp[i + 1][0] = max(dp[i][0] + base * count, dp[i + 1][0]);

      if (!flag) continue;
      dp[i + 1][1] = max(dp[i][0] + base * bits[60 - i], dp[i + 1][1]);
      continue;
    }

    if (dp[i][1] < 0) continue;
    dp[i + 1][1] = max(dp[i][1] + base * bits[60 - i], dp[i + 1][1]);
    dp[i + 1][1] = max(dp[i][1] + base * (n - bits[60 - i]), dp[i + 1][1]);
  }

  put(max(dp[61][0], dp[61][1]));
  return 0;
}
