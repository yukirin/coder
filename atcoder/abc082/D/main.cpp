...
int main(int argc, char* argv[]) {
  ...
  vector<vector<ll>> move(2);
  int index = 0, move_count = 0;
  rep(i, 0, sz(s)) {
    char command = s[i];
    if (command == 'T') {
      move[index % 2].pb(move_count);
      move_count = 0;
      index++;
      continue;
    }
  ...
  if (move_count > 0) move[index % 2].pb(move_count);

  bool ans = true;
  Fill(dp1, false);
  Fill(dp2, false);
  dp1[8000] = true;
  bool *cur = dp1, *nxt = dp2;
  rep(i, 0, sz(move[0])) {
    int dist = move[0][i];
    rep(j, 0, 16001) {
      if (!cur[j]) continue;
