...
static constexpr const char* YES = "POSSIBLE";
static constexpr const char* NO = "IMPOSSIBLE";
...
int main(int argc, char* argv[]) {
  ...
  vec<bool> middle(n), last(n);
  rep(i, 0, m) {
    ll from = a[i], to = b[i];
    if (from == 0) middle[to] = true;
    if (to == n - 1) last[from] = true;
  }
