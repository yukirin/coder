...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < N; i++) {
    int s, t, c;
    cin >> s >> t >> c;
    tv[s].emplace_back(t, c);
  }

  vector<i_i> ans;

  rep(i, 0, 100000) {
    if (sz(tv[i]) == 0) continue;
    rep(j, 0, sz(tv[i])) {
      int e = tv[i][j].first, chan = tv[i][j].second;
      int push_k = I_INF;
      rep(k, 0, sz(ans)) {
        if (ans[k].second == chan && ans[k].first == i) {
          push_k = k;
          break;
        }
        if (ans[k].first < i) push_k = k;
      }

      if (push_k == I_INF) {
        ans.emplace_back(e, chan);
        continue;
      }

      ans[push_k] = make_pair(e, chan);
    }
  }
