...
int main(int argc, char* argv[]) {
  int n;
  cin >> n;
  vector<vector<int>> shop(n, vector<int>(10));
  rep(i, 0, n) scan(shop[i]);
  vector<vector<ll>> profit(n, vector<ll>(11));
  rep(i, 0, n) scan(profit[i]);
  ...
  rep(i, 1, (1 << 10)) {
    ...
    rep(j, 0, n) {
      int count = 0;
      rep(k, 0, 10) { count += (i & (1 << k)) && shop[j][k]; }
      total += profit[j][count];
    }
    chmax(ans, total);
  }
