...
int main(int argc, char* argv[]) {
  ll n, k;
  cin >> n >> k;
  vector<ll_ll> sushis(n);
  for (auto& p : sushis) {
    cin >> p.second >> p.first;
  }

  sort(all(sushis), greater<ll_ll>());
  map<ll, ll> m;
  ll total = 0, kinds = 0;
  rrep(i, k, 0) {
    if (m[sushis[i].second] == 0) kinds++;
    m[sushis[i].second]++;
    total += sushis[i].first;
  }

  ll ans = total + kinds * kinds;
  int next_index = k;
  rep(i, k, n) {
    if (kinds >= k) break;
    if (m[sushis[i].second]) continue;

    rrep(j, next_index, 0) {
      if (m[sushis[j].second] == 1) continue;
      m[sushis[j].second]--;
      m[sushis[i].second]++;
      total -= sushis[j].first;
      total += sushis[i].first;
      kinds++;
      next_index = j;
      break;
