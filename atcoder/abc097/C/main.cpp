...
int main(int argc, char* argv[]) {
  ...
  auto f = [](auto x, auto y) { return x.first > y.first; };
  priority_queue<pair<string, int>, vector<pair<string, int>>, decltype(f)> q(f);
  rep(i, 0, sz(s)) q.push({string{s[i]}, i});
  ...
  while (count < K) {
    ...
    if (w.second == sz(s) - 1) continue;
    q.push({w.first + s[w.second + 1], w.second + 1});
  }
