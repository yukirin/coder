...
int main(int argc, char* argv[]) {
  ...
  cin >> h >> w;
  vector<string> m(h);
  vector<vector<int>> history(h, vector<int>(w));
  rep(i, 0, h) cin >> m[i];

  int white = 0;
  rep(i, 0, h) rep(j, 0, w) white += m[i][j] == '.';

  queue<i_i> q;
  q.push({0, 0});
  history[0][0] = 1;

  vector<i_i> d{{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
  while (!q.empty()) {
    auto p = q.front();
    q.pop();
    ...
    for (auto dp : d) {
      if (p.first == 0 && dp.first == -1) continue;
      if (p.first == h - 1 && dp.first == 1) continue;
      if (p.second == 0 && dp.second == -1) continue;
      if (p.second == w - 1 && dp.second == 1) continue;

      i_i next_p{p.first + dp.first, p.second + dp.second};
      if (m[next_p.first][next_p.second] == '#') continue;
      if (history[next_p.first][next_p.second] != 0) continue;

      q.push(next_p);
      history[next_p.first][next_p.second] = history[p.first][p.second] + 1;
    }
  }

  if (history[h - 1][w - 1] == 0) {
    put(-1);
    return 0;
  }

  put(white - history[h - 1][w - 1]);
  return 0;
}
