...
int main(int argc, char* argv[]) {
  ...
  ll cur_time = 0;
  rep(i, 0, N) {
    ll_ll next_pos{x[i], y[i]};
    ll next_time = t[i];
    ll move_cost = abs(next_pos.first - cur_pos.first) + abs(next_pos.second - cur_pos.second);
    ll duration = next_time - cur_time;

    if (duration - move_cost < 0) {
      put(NO);
      return 0;
    }

    if ((duration - move_cost) % 2 == 1) {
      put(NO);
      return 0;
    }
