...
bool comp(int (&a)[10], int (&b)[10]) {
  int a_count = accumulate(all(a), -1);
  int b_count = accumulate(all(b), -1);

  if (a_count > b_count) return true;
  if (b_count > a_count) return false;
  rrep(i, 10, 1) {
...
int main(int argc, char* argv[]) {
  ...
  rep(i, 0, n + 1) {
    rep(j, 0, m) {
      int match_num = match[a[j]];
      if (i - match_num < 0) continue;
      if (!dp[i - match_num][0]) continue;

      dp[i - match_num][a[j]]++;
      if (comp(dp[i - match_num], dp[i])) copy(all(dp[i - match_num]), dp[i]);
      dp[i - match_num][a[j]]--;
    }
  }

  string ans = "";
  rrep(i, 10, 1) ans += string(dp[n][i], '0' + i);
  put(ans);
  return 0;
}
