...
int main(int argc, char* argv[]) {
  ...
  pair<ll, ll> votes{t[0], a[0]};
  rep(i, 1, n) {
    pair<ll, ll> ratio{t[i], a[i]};
    ll multi = max((votes.first + (ratio.first - 1)) / ratio.first,
                   (votes.second + (ratio.second - 1)) / ratio.second);
    votes.first = ratio.first * multi;
    votes.second = ratio.second * multi;
  }

  put(ll(votes.first + votes.second));
  return 0;
}
