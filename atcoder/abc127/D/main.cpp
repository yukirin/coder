...
int main(int argc, char* argv[]) {
  ...
  scan(cards);
  vector<ll_ll> ps(m);
  rep(i, 0, m) { cin >> ps[i].second >> ps[i].first; }

  sort(all(cards));
  sort(all(ps), greater<ll_ll>());

  ll ans = 0;
  auto first = begin(cards);
  auto last = end(cards);
  int index = 0;
  for (ll_ll p : ps) {
    ll num = p.second;
    ...
    auto itr = lower_bound(first, last, value);
    int count = min(int(itr - first), int(num));
    index += count;
    ans += value * count;
    first += count;
  }

  rep(i, index, n) { ans += cards[i]; }
  put(ans);
  return 0;
}
