...
static constexpr ll LL_INF = 1LL << 62;
...
int main(int argc, char* argv[]) {
  ...
  sort(all(p), [](auto a, auto b) { return a.first < b.first; });

  ll ans = LL_INF;
  rep(i, 0, n - k + 1) rep(j, i + k, n + 1) {
    vec<ll_ll> candidate(begin(p) + i + 1, begin(p) + j - 1);
    sort(all(candidate), [](auto a, auto b) { return a.second < b.second; });

    if (k == 2) {
      ll x = abs(p[i].first - p[j - 1].first);
      ll y = abs(p[i].second - p[j - 1].second);
      chmin(ans, x * y);
      continue;
    }

    ll x = abs(p[i].first - p[j - 1].first);
    ll lower_y = min(p[i].second, p[j - 1].second), upper_y = max(p[i].second, p[j - 1].second);
    rep(m, 0, sz(candidate) - (k - 2) + 1) {
      ll y = abs(min(lower_y, candidate[m].second) - max(upper_y, candidate[m + (k - 2) - 1].second));
      chmin(ans, x * y);
    }
  }
