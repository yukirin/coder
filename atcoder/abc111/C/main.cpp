...
int main(int argc, char* argv[]) {
  ...
  map<ll, ll> am;
  for (int i = 0; i < n; i += 2) am[v[i]]++;
  map<ll, ll> bm;
  for (int i = 1; i < n; i += 2) bm[v[i]]++;

  vector<ll_ll> a;
  vector<ll_ll> b;
  for (auto p : am) a.pb(p);
  for (auto p : bm) b.pb(p);
  sort(all(a), [](auto& pa, auto& pb) { return pa.second < pb.second; });
  sort(all(b), [](auto& pa, auto& pb) { return pa.second < pb.second; });

  if (a[sz(a) - 1].first == b[sz(b) - 1].first) {
    ll x = LL_INF, y = LL_INF;
    if (sz(b) > 1) x = (n / 2) - a[sz(a) - 1].second + (n / 2) - b[sz(b) - 2].second;
    if (sz(a) > 1) y = (n / 2) - b[sz(b) - 1].second + (n / 2) - a[sz(a) - 2].second;
  ...
  ll ans = 0;
  ans += (n / 2) - a[sz(a) - 1].second;
  ans += (n / 2) - b[sz(b) - 1].second;
  put(ans);
