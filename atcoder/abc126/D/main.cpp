...
void dfs(int num, ll weight, vector<Node>& nodes) {
  if (nodes[num].checked) return;
  nodes[num].color = weight % 2;
  nodes[num].checked = true;

  for (pair<int, ll> p : nodes[num].edges) {
    dfs(p.first, weight + p.second, nodes);
  }
...
int main(int argc, char* argv[]) {
  ...
  rep(i, 0, n - 1) {
    ...
    cin >> n1 >> n2 >> w;
    n1--, n2--;
    nodes[n1].edges.pb({n2, w});
    nodes[n2].edges.pb({n1, w});
  }
