...
int main(int argc, char* argv[]) {
  long long N, total = 0;
  scanf("%lld", &N);
  std::vector<long long> A(N + 1), skips(N);
  for (int i = 1; i < N + 1; i++) scanf("%lld", &A[i]);
  A.pb(0);

  rep(i, 0, N + 1) { total += abs(A[i] - A[i + 1]); }
  rep(i, 0, N) { skips[i] = abs(A[i] - A[i + 2]); }
  rep(i, 0, N) put(total + skips[i] - abs(A[i] - A[i + 1]) - abs(A[i + 1] - A[i + 2]));
  return 0;
}
