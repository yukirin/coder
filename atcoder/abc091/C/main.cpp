...
int main(int argc, char* argv[]) {
  long long N;
  scanf("%lld", &N);
  vector<ll_ll> red(N), blue(N);
  for (int i = 0; i < N; i++) {
    scanf("%lld", &red[i].first);
    scanf("%lld", &red[i].second);
  }
  std::vector<long long> c(N);
  std::vector<long long> d(N);
  for (int i = 0; i < N; i++) {
    scanf("%lld", &blue[i].first);
    scanf("%lld", &blue[i].second);
  }
  ...
  ll ans = 0;
  vector<bool> checked(2 * N, true);
  rep(i, 0, N) {
    auto p = lower_bound(all(red), blue[i]);
    // vector<ll_ll> candidates(begin(red), p);
    // sort(all(candidates), [](auto a, auto b) { return a.second > b.second; });

    // for (auto p : candidates) {
    //   if (!checked[p.first]) continue;
    //   if (p.second > blue[i].second) continue;
    //   checked[p.first] = false;
    //   ans++;
    //   break;
    // }

    ll_ll c{-LL_INF, -LL_INF};
    for (auto s = begin(red); s != p; s++) {
      ll x = (*s).first, y = (*s).second;
      if (!checked[x]) continue;
      if (y > blue[i].second) continue;
      if (y < c.second) continue;
      c.first = x, c.second = y;
    }

    if (c.first == -LL_INF) continue;
    ans++;
    checked[c.first] = false;
  }
