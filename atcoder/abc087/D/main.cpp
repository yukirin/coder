...
int main(int argc, char* argv[]) {
  ...
  map<ll, vector<ll_ll>> info;
  vector<ll> dists(N, LL_INF);
  rep(i, 0, M) {
    ll l = L[i];
    ll r = R[i];
    ll d = D[i];
  ...
  while (!q.empty() || index < N) {
    if (q.empty()) {
      for (; index < N; index++) {
        if (dists[index] != LL_INF) continue;
        dists[index] = 0;
        q.push(index);
        break;
      }
    ...
    rep(i, 0, sz(info[l])) {
      ll r = info[l][i].first;
      ll d = info[l][i].second;

      if (dists[r] == LL_INF) {
        dists[r] = dists[l] + d;
        q.push(r);
      }
      if (dists[r] != dists[l] + d) {
        put(NO);
        return 0;
      }
    }
  ...
  ll min_d = *min_element(all(dists));
  ll max_d = *max_element(all(dists));
  if (abs(max_d - min_d) > 1e9) {
    put(NO);
    return 0;
  }
