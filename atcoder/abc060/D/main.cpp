...
int main(int argc, char* argv[]) {
  ...
  ll w1 = weight[0];
  for (ll& val : weight) val -= w1;

  Fill(dp, -1LL);
  dp[0][0][0] = 0;
  rep(i, 0, n) rep(j, 0, n) rep(total, 0, 301) {
    if (dp[i][j][total] == -1) continue;
    chmax(dp[i + 1][j][total], dp[i][j][total]);
    chmax(dp[i + 1][j + 1][total + weight[i]], dp[i][j][total] + value[i]);
  }

  ll ans = 0;
  rep(i, 0, n + 1) rep(total, 0, 301) {
    ll total_weight = i * w1 + total;
    if (total_weight > w) continue;
    chmax(ans, dp[n][i][total]);
  }
