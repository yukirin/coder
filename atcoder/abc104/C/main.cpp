...
ll dfs(ll n, vector<bool>& checked) {
  if (n == D) {
    ll score = 0, ret = 0;
    rep(i, 0, D) {
    ...
    if (score >= G) return ret;
    rrep(i, D, 0) {
      if (checked[i]) continue;
      if ((G - score) > (i + 1) * 100 * (p[i] - 1)) return LL_INF;
      ret += ll(ceil(double(G - score) / double((i + 1) * 100)));
      return ret;
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  vector<bool> checked(D);
  ll ans = dfs(0, checked);
  put(ans);
  return 0;
}
