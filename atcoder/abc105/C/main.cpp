...
int main(int argc, char* argv[]) {
  ...
  vector<ll_ll> mm{{0, 1}};
  ll base = 1;
  while (n < mm[sz(mm) - 1].first || n > mm[sz(mm) - 1].second) {
    base *= -2;
    if (base < 0) {
      mm.pb({mm[sz(mm) - 1].first + base, mm[sz(mm) - 1].second});
      continue;
    }

    mm.pb({mm[sz(mm) - 1].first, mm[sz(mm) - 1].second + base});
  }

  int index = sz(mm);
  string ans = "";
  while (index != 1) {
    if (n >= mm[index - 1].first && n < mm[index - 2].first) {
      ans += "1";
      index--;
      n -= ll(pow(-2, index));
      continue;
    ...
    if (n <= mm[index - 1].second && n > mm[index - 2].second) {
      ans += "1";
      index--;
      n -= ll(pow(-2, index));
      continue;
